const STORAGE_KEY = "godsaeng-routine-final-v4";
...
function renderRoutines() {
  ...
  state.routines.forEach((routine) => {
    ...
    deleteBtn.onclick = () => {
      ...
      Object.values(state.checks).forEach((day) => delete day[routine.id]);
      save();
      renderHome();
      renderRecord();
    };
    ...
    item.querySelector(".check-btn").onclick = (e) => {
      ...
      const wasDone = !!state.checks[key][routine.id];
      state.checks[key][routine.id] = !wasDone;

      if (!wasDone) vibrate(22);
    ...
    item.addEventListener("touchend", () => {
      ...
      if (diff < -35) wrap.classList.add("open");
      if (diff > 35) wrap.classList.remove("open");
    });
    ...
    wrap.addEventListener("dragover", (e) => {
      ...
      const toIndex = state.routines.findIndex((r) => r.id === targetId);
...
function renderCalendar(year, month, lastDay) {
  ...
  for (let day = 1; day <= lastDay; day++) {
    ...
    el.onclick = () => {
      ...
      document.querySelector('[data-tab="homeScreen"]').classList.add("active");
